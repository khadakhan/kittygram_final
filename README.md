...
# Kittygram:
Kittygram is a service developed for those who have cats. In Kittygram, each cat owner can show other registered users their pet. To do this, the cat owner must:
* register
* authenticate
* click the "Add a cat" button
* upload a photo of your cat (optional)
* indicate the cat's name
* indicate the year of birth of the cat
* select the most suitable color from the color palette
* come up with and indicate the achievement of your cat (optional)

Each cat owner can indicate several cats if they have a cat farm at home. If desired, the cat owner can delete a cat from the service or edit the cat's data.
...
## Building the Kittygram backend image.
Make sure Docker is running. Open a terminal, go to the backend/ directory of the Kittygram project and build the image:
```
...
Perform migrations in the kittygram_backend_container container:
```
docker exec kittygram_backend_container python manage.py migrate
```
Stop the container with Ctrl+C in the same terminal or with the command docker container stop kittygram_backend_container in a new terminal window. This will remove the container.

## Building the kittygram frontend image.
The Dockerfile must be placed in the frontend/ folder of the kittygram project.
...
```
docker run --rm -it -p 8000:8000 --name kittygram_frontend_test kittygram_frontend
```
Open a browser and make sure that the kittygram frontend is accessible at http://127.0.0.1:8000/. The frontend SPA does not interact with the backend yet: for this, you will need to configure them to run together; however, the web page opens successfully.
...
Rebuild the kittygram_backend image with the updated dependencies file: in the backend/ directory, run the docker build command:
```
docker build -t kittygram_backend .
```
...
## Backend statics
First, in the kittygram, configure the collection of statics in the collected_static/ folder. In the settings.py file, check the value of the STATIC_URL constant and add the STATIC_ROOT constant:
```
...
```
When accessing http://localhost:8000/ you should see the kittygram home page, which should now be fully functional. Test this by adding a task through the home page interface.
...
remove a container
```
docker container rm kittygram_backend_container
```
...
docker run --name kittygram_backend_container --rm -p 8000:8000 kittygram_backend
```
Run migrations in the kittygram_backend_container container:
```
docker exec kittygram_backend_container python manage.py migrate
```
Delete volume
...
Copy the docker-compose.production.yml file to the kittygram/ directory on the server. You can do this, for example, using the SCP (secure copy) utility — it is designed to copy files between computers. Go to the kittygram/ directory on your computer and run the copy command:
```
...
Earlier we have already deployed the kittygram application manually on the server without using docker, so the nginx settings remain:
```
...
Create a .github/workflows directory in the kittygram/ folder, and in it — a main.yml file like in the repository.
